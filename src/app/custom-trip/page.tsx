...
import { FaUser, FaUsers, FaEnvelope, FaWhatsapp, FaGlobe } from 'react-icons/fa';
...
export default function AIBasedTripPlanner() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedDate || !name || !email || !whatsapp || !country || passengers < 1) {
  ...
  const nextStep = () => {
    ...
    if (currentStep === 2 && (!name || !email || !whatsapp || !country || passengers < 1 || !selectedDate)) {
        alert('Please provide all your details and select a date.');
        return;
    }
                    ...
                    <motion.div key="step2" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.5 }} className="space-y-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full rounded-xl border-2 border-foreground/20 bg-background/80 p-3 text-foreground focus:ring-yellow-400 focus:border-yellow-400" />
                            </div>
                            <div>
                                <label htmlFor="whatsapp" className="block text-lg font-bold text-foreground mb-3">
                                    <FaWhatsapp className="inline mr-2" /> WhatsApp Number
                                </label>
                                <input id="whatsapp" type="tel" value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} required className="w-full rounded-xl border-2 border-foreground/20 bg-background/80 p-3 text-foreground focus:ring-yellow-400 focus:border-yellow-400" />
                            </div>
                            <div>
                                <label htmlFor="country" className="block text-lg font-bold text-foreground mb-3">
                                    <FaGlobe className="inline mr-2" /> Country
                                </label>
                                <input id="country" type="text" value={country} onChange={(e) => setCountry(e.target.value)} required className="w-full rounded-xl border-2 border-foreground/20 bg-background/80 p-3 text-foreground focus:ring-yellow-400 focus:border-yellow-400" />
                            </div>
                            <div>
                    ...
                    <motion.div key="step3" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.5 }} className="space-y-6">
                        <div className="bg-foreground/5 border border-foreground/10 rounded-xl p-8">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-between py-3 border-b border-foreground/10"><span className="text-foreground/70 font-medium">📧 Email:</span><span className="text-foreground font-semibold">{email}</span></div>
                                <div className="flex justify-between py-3 border-b border-foreground/10"><span className="text-foreground/70 font-medium">📞 WhatsApp:</span><span className="text-foreground font-semibold">{whatsapp}</span></div>
                                <div className="flex justify-between py-3 border-b border-foreground/10"><span className="text-foreground/70 font-medium">🌍 Country:</span><span className="text-foreground font-semibold">{country}</span></div>
